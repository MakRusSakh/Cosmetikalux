...
> Версия: 1.1
> Дата: 2026-03-18
> Обновлено: 2026-03-18 — ИИ вынесен в отдельную финальную фазу, базовый фундамент закладывается сразу
...
> **Стратегия:** ИИ-консультант вынесен в отдельную финальную фазу.
> Базовый фундамент для ИИ (модели БД, типы, API-заглушки, UI-каркас чата)
> закладывается с первой фазы — для бесшовного внедрения позже.

### Фаза 1: Фундамент (2 недели)

- [ ] Инициализация Next.js проекта
- [ ] Настройка Tailwind с дизайн-токенами из DESIGN_SYSTEM.md
- [ ] Подключение PostgreSQL + Prisma
- [ ] Модели данных + миграции (ВСЕ модели, включая ChatSession/ChatMessage/SkinProfile)
- [ ] Базовые UI-компоненты (кнопки, инпуты, карточки)
- [ ] Layout: Header + Footer + навигация
- [ ] Аутентификация (NextAuth)
- [ ] 🔧 **ИИ-фундамент:** модели ChatSession, ChatMessage, SkinProfile в Prisma
- [ ] 🔧 **ИИ-фундамент:** типы TypeScript для чата (src/types/chat.ts)
- [ ] 🔧 **ИИ-фундамент:** пустой chatStore.ts в Zustand (интерфейс готов)
- [ ] 🔧 **ИИ-фундамент:** env-переменные GigaChat (пустые, но задокументированы)
...
- [ ] 🔧 **ИИ-фундамент:** поле concerns[], skinTypes[] на товарах (заполняется)
- [ ] 🔧 **ИИ-фундамент:** API-эндпоинт поиска товаров по характеристикам (будет использован RAG)
...
### Фаза 4: Личный кабинет (1.5 недели)
...
- [ ] Адреса доставки
- [ ] Уведомления
- [ ] 🔧 **ИИ-фундамент:** форма «Профиль кожи» (SkinProfile) — данные сохраняются, но пока не используются ИИ
- [ ] 🔧 **ИИ-фундамент:** страница /account/consultations (заглушка «Скоро»)
- [ ] 🔧 **ИИ-фундамент:** страница /account/routine (заглушка «Скоро»)
...
- [ ] 🔧 **ИИ-фундамент:** кнопка «Спросить консультанта» на карточке товара (ведёт на заглушку)
- [ ] 🔧 **ИИ-фундамент:** placeholder-виджет чата (иконка ✨ в углу → «Скоро! Персональный beauty-консультант»)

### ═══ PRODUCTION LAUNCH ═══

> Сайт запускается и работает полноценно без ИИ.
> Все покупательские сценарии функционируют.

### Фаза 8: ИИ Beauty-консультант (2-3 недели) — ОТДЕЛЬНЫЙ ЭТАП

> **Предусловия:** Фазы 1-7 завершены, сайт в production, есть реальные товары и заказы.
> **Зависимости:** GigaChat API ключи, наполненный каталог с описаниями/составами.

**Этап 8.1 — Бэкенд ИИ (1 неделя)**
- [ ] Интеграция GigaChat API (аутентификация, токены, кэш в Redis)
- [ ] Абстрактный AI Provider (интерфейс для замены GigaChat → другую LLM)
- [ ] RAG-пайплайн: извлечение ключевых слов → SQL-поиск → формирование контекста
- [ ] Системный промпт «Лина» + шаблоны контекста
- [ ] API: POST /api/chat/message (SSE streaming)
- [ ] API: POST /api/chat/session, GET /api/chat/history
- [ ] Rate limiting (10 msg/min на юзера)
- [ ] Логирование всех диалогов

**Этап 8.2 — Фронтенд чата (1 неделя)**
- [ ] ChatWidget.tsx (полный UI, замена заглушки)
- [ ] ChatMessage.tsx (bubble-сообщения с markdown)
- [ ] ChatProductCard.tsx (встроенные карточки товаров с «В корзину»)
- [ ] ChatInput.tsx + QuickButtons.tsx
- [ ] SSE-стриминг ответов (печатание в реальном времени)
- [ ] Интеграция с chatStore (Zustand)
- [ ] Мобильная версия (fullscreen)
- [ ] Кнопка «Спросить консультанта» на карточке товара → чат с контекстом

**Этап 8.3 — Персонализация и рутины (0.5-1 неделя)**
- [ ] Подключение SkinProfile к контексту GigaChat
- [ ] Подключение истории заказов к контексту
- [ ] Рутина ухода (/account/routine) — ИИ генерирует план из купленных товаров
- [ ] История консультаций (/account/consultations)
- [ ] Рекомендации на главной: «Подобрано для вас» (на основе профиля кожи)

**Этап 8.4 — Админка ИИ + оптимизация (0.5 недели)**
- [ ] /admin/chat — просмотр всех сессий
- [ ] /admin/chat/system-prompt — редактирование промпта без деплоя
- [ ] /admin/chat/analytics — метрики: кол-во диалогов, конверсия в покупку, популярные вопросы
- [ ] A/B тестирование промптов
- [ ] Оптимизация: кэширование частых ответов, fallback при недоступности GigaChat

---

## 12. Критерии приёмки

### Фазы 1-7 (Production Launch)
...
- [ ] Профиль кожи заполняется и сохраняется
- [ ] ИИ-заглушки отображаются корректно («Скоро»)
...
- [ ] 99.5% uptime
- [ ] ИИ-фундамент: все модели/типы/store готовы к подключению

### Фаза 8 (ИИ-консультант)

**Функциональные:**
- [ ] Чат-виджет открывается, стримит ответы
- [ ] ИИ рекомендует товары из каталога (не выдуманные)
- [ ] Карточки товаров в чате кликабельны, кнопка «В корзину» работает
- [ ] Профиль кожи влияет на рекомендации
- [ ] Рутина ухода генерируется
- [ ] Кнопка «Спросить консультанта» на товаре передаёт контекст
- [ ] Админ может менять системный промпт без деплоя

**Технические:**
- [ ] Время первого ответа ИИ < 3 секунды
- [ ] Rate limiting работает (10 msg/min)
- [ ] Fallback при недоступности GigaChat (вежливое сообщение)
- [ ] Все диалоги логируются
- [ ] AI Provider абстрагирован (можно заменить GigaChat на другую LLM)

---

> **Статус:** ТЗ v1.1 — ИИ вынесен в отдельную фазу, фундамент закладывается сразу.
